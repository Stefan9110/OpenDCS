"use client"

import type {Project, ProjectRole} from "@/lib/api/types"
import {notifyComingSoon} from "@/lib/notify"
import {formatUpdatedAt} from "@/lib/projects"
import {logoGradient} from "@/theme/theme"
import {ActionIcon, Badge, Card, Group, Menu, Text, UnstyledButton} from "@mantine/core"
import {type Icon, IconDots, IconEye, IconFolder, IconHome, IconPencil, IconShare, IconTrash} from "@tabler/icons-react"
import classes from "./ProjectCard.module.css"


const roleBadges: Record<ProjectRole, Icon> = {
    owner: IconHome,
    editor: IconPencil,
    viewer: IconEye,
}

export function ProjectCard({project}: Readonly<{ project: Project }>) {
    const RoleIcon = roleBadges[project.role]
    return (
        <Card withBorder radius="md" padding="md" className={classes.card} __vars={{"--logo-gradient": logoGradient}}>
            <Group justify="space-between" mb="xs">
                <IconFolder size={22}/>
                <Group gap="xs">
                    <Badge variant="light" color="opendc" leftSection={<RoleIcon size={14}/>}>
                        {project.role}
                    </Badge>
                    <ProjectActions project={project}/>
                </Group>
            </Group>
            <UnstyledButton onClick={() => notifyComingSoon("Opening a project")}>
                <Text fw={600} lineClamp={1}>
                    {project.name}
                </Text>
                <Text size="sm" c="dimmed" mt={4}>
                    Last modified {formatUpdatedAt(project.updatedAt)}
                </Text>
            </UnstyledButton>
        </Card>
    )
}

function ProjectActions({project}: Readonly<{ project: Project }>) {
    const isOwner = project.role === "owner"
    return (
        <Menu position="bottom-end">
            <Menu.Target>
                <ActionIcon variant="subtle" color="gray" aria-label="Project actions">
                    <IconDots size={18}/>
                </ActionIcon>
            </Menu.Target>
            <Menu.Dropdown>
                {isOwner && (
                    <Menu.Item
                        leftSection={<IconShare size={16}/>}
                        onClick={() => notifyComingSoon("Sharing a project")}
                    >
                        Share
                    </Menu.Item>
                )}
                <Menu.Item
                    color="red"
                    leftSection={<IconTrash size={16}/>}
                    onClick={() => notifyComingSoon("Deleting a project")}
                >
                    Delete
                </Menu.Item>
            </Menu.Dropdown>
        </Menu>
    )
}
